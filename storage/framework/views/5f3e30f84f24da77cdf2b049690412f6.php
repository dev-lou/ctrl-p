...
<div style="position: fixed; top: 16px; left: 16px; right: 16px; z-index: 50;">
    <div style="max-width: 80rem; margin: 0 auto;">
        ...
        <div class="flex items-center">
            <a href="/" class="text-2xl font-bold text-black" style="font-family: 'Inter', sans-serif; font-size: 24px;"><?php echo e(config('app.name', 'CICT')); ?></a>
        </div>
        ...
        <div class="hidden md:flex items-center gap-12">
            ...
            <a href="<?php echo e(route('shop.index')); ?>" class="navbar-link <?php echo e(request()->routeIs('shop.*') ? 'active' : ''); ?>">
                Shop
            </a>
            <a href="<?php echo e(route('services.index')); ?>" class="navbar-link <?php echo e(request()->routeIs('services.*') ? 'active' : ''); ?>">
                Services
            </a>
            <a href="<?php echo e(route('contact.index')); ?>" class="navbar-link <?php echo e(request()->routeIs('contact.*') ? 'active' : ''); ?>">
                Contact
            </a>
        </div>
        ...
        <div class="flex items-center gap-4">
            ...
            <?php if(auth()->guard()->check()): ?>
                <a href="<?php echo e(route('cart.index')); ?>" class="relative p-2.5 rounded-lg transition-all duration-200 hover:bg-black/10 text-black touch-target" title="Shopping Cart">
                    ...
                    ?>
                    <?php if($cartCount > 0): ?>
                        <span class="cart-count-badge cart-count-badge--nav animate-pulse"><?php echo e($cartCount); ?></span>
                    <?php endif; ?>
                </a>
                ...
                <div x-data="{ menuOpen: false }" class="relative hidden md:flex">
                    ...
                    >
                        <?php if(auth()->user()->profile_picture): ?>
                            <img src="<?php echo e(asset('storage/' . auth()->user()->profile_picture)); ?>" alt="<?php echo e(auth()->user()->name); ?>" class="w-8 h-8 rounded-full object-cover border-2 border-gray-200">
                        <?php else: ?>
                    ...
                    >
                        ...
                        <div class="dropdown-header px-4 py-4 flex items-center gap-3" style="border-bottom: 1px solid rgba(0, 0, 0, 0.08); background: linear-gradient(135deg, rgba(218, 165, 32, 0.12) 0%, rgba(218, 165, 32, 0.06) 100%);">
                            <?php if(auth()->user()->profile_picture): ?>
                                <img src="<?php echo e(asset('storage/' . auth()->user()->profile_picture)); ?>" alt="<?php echo e(auth()->user()->name); ?>" class="w-12 h-12 rounded-full object-cover border-2 border-white shadow-sm">
                            <?php else: ?>
                            ...
                            <?php endif; ?>
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-bold text-black truncate"><?php echo e(auth()->user()->name); ?></p>
                                <p class="text-xs text-gray-600 truncate"><?php echo e(auth()->user()->email); ?></p>
                            </div>
                        </div>
                        
                        <?php if(auth()->user()->isAdmin()): ?>
                            <a href="<?php echo e(route('admin.dashboard')); ?>" class="dropdown-item block px-4 py-3 text-sm font-semibold text-black transition-colors flex items-center gap-3" style="border-bottom: 1px solid rgba(0, 0, 0, 0.05);">
                                <span class="text-lg">🛠️</span>
                                <span>Admin Dashboard</span>
                            </a>
                        <?php endif; ?>

                        <a href="<?php echo e(route('profile.show')); ?>" class="dropdown-item block px-4 py-3 text-sm font-semibold text-black transition-colors flex items-center gap-3">
                            <span class="text-lg">👤</span>
                            <span>My Profile</span>
                        </a>
                        
                        <a href="<?php echo e(route('account.orders')); ?>" class="dropdown-item block px-4 py-3 text-sm font-semibold text-black transition-colors flex items-center gap-3" style="border-bottom: 1px solid rgba(0, 0, 0, 0.05);">
                            <span class="text-lg">📦</span>
                            <span>My Orders</span>
                        </a>
                        
                        <form method="POST" action="<?php echo e(route('logout')); ?>" class="block" style="border-top: 1px solid rgba(0, 0, 0, 0.08);">
                            <?php echo csrf_field(); ?>
...
>
    ...
    <?php if(auth()->guard()->check()): ?>
        ...
        <div class="p-4 border-b border-gray-200" style="background: linear-gradient(135deg, rgba(139, 0, 0, 0.05) 0%, transparent 100%);">
            <div class="flex items-center gap-3">
                <?php if(auth()->user()->profile_picture): ?>
                    <img src="<?php echo e(asset('storage/' . auth()->user()->profile_picture)); ?>" alt="Profile" class="w-12 h-12 rounded-lg object-cover border-2 border-gray-300">
                <?php else: ?>
    ...
    <nav class="py-2">
        <a 
            href="/" 
            @click="closeMobileMenu()"
            class="mobile-nav-link <?php echo e(request()->is('/') ? 'active' : ''); ?>"
        >
        ...
        <a 
            ...
            @click="closeMobileMenu()"
            class="mobile-nav-link <?php echo e(request()->routeIs('shop.*') ? 'active' : ''); ?>"
        >
            <svg class="mobile-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <a 
            ...
            @click="closeMobileMenu()"
            class="mobile-nav-link <?php echo e(request()->routeIs('services.*') ? 'active' : ''); ?>"
        >
            <svg class="mobile-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
</div><?php /**PATH C:\xampp\htdocs\laravel_igp\resources\views\components\navbar.blade.php ENDPATH**/ ?>