<x-admin-layout>
    ...
    <div style="display: grid; grid-template-columns: 1fr 360px; gap: 28px; margin-bottom: 40px;" class="main-container">
        ...
        <div style="min-width: 0;">
            ...
            <form id="productForm" action="{{ route('admin.inventory.update', $product) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PATCH')
                ...
                <div class="section-container">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="name" value="{{ $product->name }}" required class="form-input" placeholder="Enter product name" />
                        @error('name')<span style="color: #ff9500; font-size: 0.95rem; font-weight: 600; margin-top: 6px; display: block;">❌ {{ $message }}</span>@enderror
                    </div>

                    <div class="form-group">
                        <label class="form-label">Description</label>
                        <textarea name="description" rows="5" class="form-input" style="resize: none; font-size: 1rem;" placeholder="Add product description...">{{ $product->description }}</textarea>
                        @error('description')<span style="color: #ff9500; font-size: 0.95rem; font-weight: 600; margin-top: 6px; display: block;">❌ {{ $message }}</span>@enderror
                    </div>

                    <div class="input-group">
                        <div class="form-group" style="margin-bottom: 0;">
                            <label class="form-label">Base Price (₱) *</label>
                            <input type="number" name="base_price" value="{{ $product->base_price }}" step="0.01" required class="form-input" placeholder="0.00" />
                            @error('base_price')<span style="color: #ff9500; font-size: 0.95rem; font-weight: 600; margin-top: 6px; display: block;">❌ {{ $message }}</span>@enderror
                        </div>
                        ...
                        <div class="form-group" style="margin-bottom: 0;">
                            ...
                            @error('status')<span style="color: #ff9500; font-size: 0.95rem; font-weight: 600; margin-top: 6px; display: block;">❌ {{ $message }}</span>@enderror
                        </div>
                    </div>

                    <div class="form-group">
                        <label class="form-label">Low Stock Threshold</label>
                        <input type="number" name="low_stock_threshold" value="{{ $product->low_stock_threshold }}" class="form-input" min="1" required>
                        <div class="info-box" style="margin-top: 8px;">
                            <span style="color: #cbd5e1;">Receive alerts when stock falls below this level</span>
                        </div>
                        @error('low_stock_threshold')<span style="color: #ff9500; font-size: 0.95rem; font-weight: 600; margin-top: 6px; display: block;">❌ {{ $message }}</span>@enderror
                    </div>
                </div>
                        ...
                        <div style="background: rgba(59, 130, 246, 0.1); border: 1px solid rgba(59, 130, 246, 0.3); border-radius: 8px; padding: 20px; margin-bottom: 24px; backdrop-filter: blur(10px);">
                            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px;">
                                <div style="text-align: center;">
                                    ...
                                    <span style="color: #3b82f6; font-weight: 700; font-size: 2rem;">{{ $product->variants->sum('stock_quantity') }}</span>
                                ...
                                <div style="text-align: center;">
                                    ...
                                    <span style="color: #ff9500; font-weight: 700; font-size: 2rem;">{{ $product->variants->where('stock_quantity', '<=', 20)->count() }}</span>
                                </div>
                                <div style="text-align: center;">
                                    <p style="color: #94a3b8; font-size: 0.85rem; margin: 0 0 6px 0;">Avg. Price Modifier</p>
                                    <span style="color: #22c55e; font-weight: 700; font-size: 2rem;">₱{{ number_format($product->variants->avg('price_modifier') ?? 0, 2) }}</span>
                                </div>
                            </div>
                        </div>
                            ...
                            <table style="width: 100%; border-collapse: collapse;">
                                ...
                                <tbody>
                                    @foreach ($product->variants as $index => $variant)
                                        <tr class="variant-row" data-variant-id="{{ $variant->id }}" style="border-bottom: 1px solid rgba(71, 85, 105, 0.2); transition: all 0.25s cubic-bezier(0.4, 0, 0.2, 1);" onmouseover="this.style.backgroundColor='rgba(59, 130, 246, 0.08)';" onmouseout="this.style.backgroundColor='transparent';">
                                            <td style="padding: 14px 16px;">
                                                <input type="text" class="variantName form-input variant-input" name="variants[{{ $index }}][name]" value="{{ $variant->name }}" placeholder="Variant name" style="padding: 10px 12px; font-size: 0.95rem;" />
                                            </td>
                                            <td style="padding: 14px 16px; text-align: center;">
                                                <input type="number" class="variantStock form-input variant-input" name="variants[{{ $index }}][stock_quantity]" value="{{ $variant->stock_quantity }}" min="0" placeholder="0" style="padding: 10px 12px; font-size: 0.95rem; text-align: center; width: 100px; margin: 0 auto;" />
                                            ...
                                            <td style="padding: 14px 16px; text-align: center;">
                                                <input type="number" class="variantPriceModifier form-input variant-input" name="variants[{{ $index }}][price_modifier]" value="{{ $variant->price_modifier }}" step="0.01" placeholder="0.00" style="padding: 10px 12px; font-size: 0.95rem; text-align: center; width: 120px; margin: 0 auto;" />
                                            </td>
                                            <td style="padding: 14px 16px; text-align: center;">
                                                <input type="text" class="variantSize form-input variant-input" name="variants[{{ $index }}][size]" value="{{ $variant->size }}" placeholder="Size" style="padding: 10px 12px; font-size: 0.95rem; text-align: center; width: 100px; margin: 0 auto;" />
                                            </td>
                                            <td style="padding: 14px 16px; text-align: center;">
                                                <input type="text" class="variantColor form-input variant-input" name="variants[{{ $index }}][color]" value="{{ $variant->color }}" placeholder="Color" style="padding: 10px 12px; font-size: 0.95rem; text-align: center; width: 120px; margin: 0 auto;" />
                                            </td>
                                            <td style="padding: 14px 16px; text-align: center;">
                                                <input type="text" class="variantWeight form-input variant-input" name="variants[{{ $index }}][weight]" value="{{ $variant->weight }}" placeholder="Weight" style="padding: 10px 12px; font-size: 0.95rem; text-align: center; width: 100px; margin: 0 auto;" />
                                            </td>
                                            <td style="padding: 14px 16px; text-align: center;">
                                                <button type="button" class="deleteVariantBtn btn-danger" data-variant-id="{{ $variant->id }}" onclick="deleteVariant(this, event);" style="padding: 8px 14px; font-size: 0.9rem; white-space: nowrap;">
                                                    🗑️
                                                </button>
                                            </td>
                                            <input type="hidden" class="variantDeletedFlag" name="variants[{{ $index }}][delete]" value="0" />
                                            <input type="hidden" class="variantIdField" name="variants[{{ $index }}][id]" value="{{ $variant->id }}" />
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                ...
                @else
                    ...
                    <div class="section-container">
                        ...
                        <div style="background: rgba(30, 41, 59, 0.4); backdrop-filter: blur(10px); border: 1px solid rgba(59, 130, 246, 0.3); border-radius: 8px; padding: 24px; margin-bottom: 24px;">
                            ...
                            <input type="number" name="current_stock" value="{{ $product->current_stock }}" min="0" required class="form-input" placeholder="0" />
                            @error('current_stock')<span style="color: #ff9500; font-size: 0.95rem; font-weight: 600; margin-top: 6px; display: block;">❌ {{ $message }}</span>@enderror

                            <div class="info-box" style="margin-top: 16px;">
                                <div style="display: flex; justify-content: space-between; align-items: center; gap: 16px;">
                                    <div>
                                        <p style="color: #94a3b8; font-size: 0.85rem; margin: 0 0 6px 0;">Total Available</p>
                                        <span style="color: #e2e8f0; font-weight: 700; font-size: 1.5rem;">{{ $product->current_stock }}</span>
                                        <span style="color: #b0bcc4; font-size: 0.95rem; margin-left: 8px;">units</span>
                                    </div>
                                    <span style="background: {{ $product->current_stock <= $product->low_stock_threshold ? '#1a7fff' : '#4caf50' }}; color: white; padding: 14px 18px; border-radius: 10px; font-weight: 700; font-size: 1.1rem; white-space: nowrap; text-align: center;">
                                        {{ $product->current_stock <= $product->low_stock_threshold ? '⚠️ Low Stock' : '✓ Adequate' }}
                                    </span>
                                </div>
                            </div>
                        </div>
